...
public class AndyFileReaderHandler {
	 ...
	 * @param callBack
	 * @param path文件保存的路径
	 * @param isResume是否断点续传
	 * @throws IOException
	 * */
	public Object readData(HttpEntity entity, AndyEntityCallBack callBack,
			String path, boolean isResume) throws IOException {
		...
		// while (!isStop && currentCount < totalCount && (readLen =
		// iStream.read(buffer, 0, 1024)) > 0) {
		 ...
		 * 
		 * 这里实际的写法应该是： current = input.skip(current); file.seek(current);
		 * 根据JDK文档中的解释：Inputstream.skip(long i)方法跳过i个字节，并返回实际跳过的字节数。
		 * 导致这种情况的原因很多，跳过 n 个字节之前已到达文件末尾只是其中一种可能。这里可能是碎片文件的损害造成的。
		 */
		file.seek(iStream.skip(currentCount));

		int readLen = 0;
		byte[] buffer = new byte[1024];
		while (!isStop && currentCount < totalCount
				&& (readLen = iStream.read(buffer, 0, 1024)) > 0) {

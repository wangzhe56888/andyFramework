...
/**
 * @description: 硬盘缓存，参考Afinal框架中的DiskCache
 * @author: andy  
 * @mail: win58@qq.com
 * @date: 2015-5-13  上午11:07:59
 */
public class AndyDiskCache implements Closeable {
     ...
     * @throws IOException
     */
    public AndyDiskCache(String path, int maxEntries, int maxBytes, boolean reset) throws IOException {
        this(path, maxEntries, maxBytes, reset, 0);
    }

    public AndyDiskCache(String path, int maxEntries, int maxBytes, boolean reset, int version) throws IOException {
    ...
    private boolean loadIndex() {
        try {
            ...
            if (mIndexFile.read(buf) != INDEX_HEADER_SIZE) {
                AndyLog.warn_bitmap("cannot read header");
                return false;
            }

            if (readInt(buf, IH_MAGIC) != MAGIC_INDEX_FILE) {
                AndyLog.warn_bitmap("cannot read header magic");
                return false;
            }

            if (readInt(buf, IH_VERSION) != mVersion) {
                AndyLog.warn_bitmap("version mismatch");
                return false;
            }
            ...
            if (checkSum(buf, 0, IH_CHECKSUM) != sum) {
                AndyLog.warn_bitmap("header checksum does not match");
                return false;
            }
            ...
            if (mMaxBytes <= 0) {
                AndyLog.warn_bitmap("invalid max bytes");
                return false;
            }
            if (mActiveRegion != 0 && mActiveRegion != 1) {
                AndyLog.warn_bitmap("invalid active region");
                return false;
            }
            if (mActiveEntries < 0 || mActiveEntries > mMaxEntries) {
                AndyLog.warn_bitmap("invalid active entries");
                return false;
            }
            if (mActiveBytes < DATA_HEADER_SIZE || mActiveBytes > mMaxBytes) {
                AndyLog.warn_bitmap("invalid active bytes");
                return false;
            }
            if (mIndexFile.length() !=
                    INDEX_HEADER_SIZE + mMaxEntries * 12 * 2) {
                AndyLog.warn_bitmap("invalid index file length");
                return false;
            }
            ...
            if (mDataFile0.read(magic) != 4) {
                AndyLog.warn_bitmap("cannot read data file magic");
                return false;
            }
            if (readInt(magic, 0) != MAGIC_DATA_FILE) {
                AndyLog.warn_bitmap("invalid data file magic");
                return false;
            }
            ...
            AndyLog.error_bitmap("loadIndex failed.", ex);
            return false;
        }
    }
        ...
        if (lookupInternal(req.key, mInactiveHashStart)) {
            if (getBlob(mInactiveDataFile, mFileOffset, req)) {
                ...
                try {
                    ...
                    AndyLog.error_bitmap("cannot copy over");
                }
                return true;
            }
        }
    ...
    private boolean getBlob(RandomAccessFile file, int offset,
        ...
        try {
            ...
            if (file.read(blob, 0, length) != length) {
                AndyLog.warn_bitmap("cannot read blob data");
                return false;
            }
            if (checkSum(blob, 0, length) != sum) {
                AndyLog.warn_bitmap("blob checksum does not match: " + sum);
                return false;
            }
            return true;
        } catch (Throwable t)  {
            AndyLog.error_bitmap("getBlob failed.", t);
            return false;
        } finally {
    ...
    private boolean lookupInternal(long key, int hashStart) {
        ...
        while (true) {
            ...
            if (candidateOffset == 0) {
                ...
                if (slot == slotBegin) {
                    AndyLog.warn_bitmap("corrupted index: clear the slot.");
    ...
    public void syncIndex() {
        try {
            ...
            AndyLog.warn_bitmap("sync index failed", t);
        }
    }
    ...
    public void syncAll() {
        ...
        try {
            ...
            AndyLog.warn_bitmap("sync data file 0 failed", t);
        }
        try {
            mDataFile1.getFD().sync();
        } catch (Throwable t) {
            AndyLog.warn_bitmap("sync data file 1 failed", t);
        }
    }

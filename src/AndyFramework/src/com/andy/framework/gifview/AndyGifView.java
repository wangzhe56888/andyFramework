...
/**
 * @description: GIF显示的view 如果要显示的gif太大，会出现OOM的问题
 * @author: andy
 * @mail: win58@qq.com
...
public class AndyGifView extends View implements AndyGifParseListener {
	...
	/** gif 解码器 **/
	private AndyGifDecoder gifDecoder = null;
	/** 当前要画的帧 **/
	...
	private GifImageShowType animationType = GifImageShowType.SYNC_DECODER;
	...
	public AndyGifView(Context context) {
		super(context);
	}

	public AndyGifView(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public AndyGifView(Context context, AttributeSet attrs, int defStyle) {
	...
	private void setGifDecoderImage(byte[] gif) {
		...
		gifDecoder = new AndyGifDecoder(gif, this);
		gifDecoder.start();
	}
	...
	private void setGifDecoderImage(InputStream is) {
		...
		gifDecoder = new AndyGifDecoder(is, this);
		gifDecoder.start();
	}
	...
	private class DrawThread extends Thread {
		public void run() {
			...
			while (isRun) {
				if (pause == false) {
					AndyGifUnit frame = gifDecoder.next();
...
/****************************** public method *********************************************/
	 ...
	 * @param type 显示方式
	 */
	public void setGifImageShowType(GifImageShowType type) {
		if (gifDecoder == null)
			animationType = type;
	}
	...
	public void parseOk(boolean parseStatus, int frameIndex) {
		if (parseStatus) {
			if (gifDecoder != null) {
				...
				case WAIT_FINISH:
					if (frameIndex == -1) {
						if (gifDecoder.getGifUnitCount() > 1) { // 当帧数大于1时，启动动画线程
				...
				case COVER:
					if (frameIndex == 1) {
						...
						if (gifDecoder.getGifUnitCount() > 1) {
							if (drawThread == null) {

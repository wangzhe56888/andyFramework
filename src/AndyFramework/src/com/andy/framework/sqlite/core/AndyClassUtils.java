...
//		return AndyFieldUtils.getFieldValue(entity,AndyClassUtils.getPrimaryKeyField(entity.getClass()));
//	}
//	
//	/**
//	 * 根据实体类 获得 实体类对应的表名
...
//	}
	...
	public static Field getPrimaryKeyField(Class<?> clazz) {
		...
		if(fields != null){
			...
			throw new AndySqliteException("this model["+clazz+"] has no id field");
		}
			...
			for (Field f : fs) {
				...
				if(!AndyFieldUtils.isTransient(f)){
					if (AndyFieldUtils.isBaseDateType(f)) {
						...
						property.setDefaultValue(AndyFieldUtils.getPropertyDefaultValue(f));
						property.setSet(AndyFieldUtils.getFieldSetMethod(clazz, f));
						property.setGet(AndyFieldUtils.getFieldGetMethod(clazz, f));
						property.setField(f);
	...
	public static List<AndyManyToOne> getManyToOneList(Class<?> clazz) {
		...
		try {
			Field[] fs = clazz.getDeclaredFields();
			for (Field f : fs) {
				if (!AndyFieldUtils.isTransient(f) && AndyFieldUtils.isManyToOne(f)) {
	...
	public static List<AndyOneToMany> getOneToManyList(Class<?> clazz) {
		...
		try {
			Field[] fs = clazz.getDeclaredFields();
			for (Field f : fs) {
				if (!AndyFieldUtils.isTransient(f) && AndyFieldUtils.isOneToMany(f)) {
					...
					if (type instanceof ParameterizedType) {
						...
						throw new AndySqliteException("getOneToManyList Exception:" + f.getName() + "'s type is null");
					}
